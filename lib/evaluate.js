...
nodeToString.op = node => `(${nodeToString(node.left)} ${OP_SYMBOLS[node.op]} ${nodeToString(node.right)})`;
...
nodeToString.roll = node => `{[${node.rolls.map(({ roll, ignored }) => ignored ? `~${roll}~` : roll).join(', ')}] -> ${node.value}}`;

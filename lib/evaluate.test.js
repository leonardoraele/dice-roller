import evaluate, { toString } from './evaluate.js';
...
describe('evaluate', () => {
	...
	test('numbers', () => {
		expect(evaluate('1')).to.deep.equal({ resolution: num(1), total: 1, expression: '1' });
		expect(evaluate('1.2')).to.deep.equal({ resolution: num(1.2), total: 1.2, expression: '1.2' });
		expect(evaluate('0')).to.deep.equal({ resolution: num(0), total: 0, expression: '0' });
		expect(evaluate('-2')).to.deep.equal({ resolution: num(-2), total: -2, expression: '-2' });
		expect(evaluate('-2.4')).to.deep.equal({ resolution: num(-2.4), total: -2.4, expression: '-2.4' });
		expect(evaluate('-0')).to.deep.equal({ resolution: num(-0), total: -0, expression: '-0' });
		expect(evaluate('.2')).to.deep.equal({ resolution: num(.2), total: .2, expression: '.2' });
	});
	...
	describe('arithmetics', () => {
		test('simple arithmetics', () => {
			expect(evaluate('1 + 3')).to.deep.equal({ resolution: sum(1, 3, 4), total: 4, expression: '1 + 3' });
			expect(evaluate('1 - 3')).to.deep.equal({ resolution: sub(1, 3, -2), total: -2, expression: '1 - 3' });
			expect(evaluate('1.2 + 0.85')).to.deep.equal({ resolution: sum(1.2, 0.85, 2.05), total: 2.05, expression: '1.2 + 0.85' });
			expect(evaluate('3 * 3')).to.deep.equal({ resolution: mul(3, 3, 9), total: 9, expression: '3 * 3' });
			expect(evaluate('9 / 3')).to.deep.equal({ resolution: div(9, 3, 3), total: 3, expression: '9 / 3' });
			expect(evaluate('2 ^ 3')).to.deep.equal({ resolution: pow(2, 3, 8), total: 8, expression: '2 ^ 3' });
			expect(evaluate('2 ** 3')).to.deep.equal({ resolution: pow(2, 3, 8), total: 8, expression: '2 ** 3' });
			expect(evaluate('1.2 ^ 3.4')).to.deep.equal({ resolution: pow(1.2, 3.4, Math.pow(1.2, 3.4)), total: Math.pow(1.2, 3.4), expression: '1.2 ^ 3.4' });
		});
	...
	test('toString', () => {
		expect(toString(evaluate('1'))).to.be.a('string');
		expect(toString(evaluate('1d20'))).to.be.a('string');
		expect(toString(evaluate('foo', { foo: 1 }))).to.be.a('string');
		// TODO
		// expect(toString(evaluate('floor(1 / 20)'))).to.be.a('string');
		expect(toString(evaluate('abs(-5)'))).to.be.a('string');
		expect(toString(evaluate('min(1d20, 1d20)'))).to.be.a('string');
		expect(toString(evaluate('2d20dl2 * mod(1d6, d4r1..2)'))).to.be.a('string');
	});
});
